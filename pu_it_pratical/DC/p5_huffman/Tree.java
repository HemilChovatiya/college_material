...
public class Tree {
 ...
 public Tree(){ root = new Node(null,null,null,' ',"-1",100,0);
 NYT=root;
 for ( int i = 0 ; i < 128 ; i++){ String code=Integer.toBinaryString(i);
 while(code.length()<8) code='0'+code;
 dic.put( ((char)i ) ,code); } }
 public String InsertSymbol(char symbol ,String out) {
 if (content.isEmpty()){ out += dic.get(symbol);
 ...
 public void UpdateInsert(Node cur){ Node r = root;
 Boolean alg1 = false; Increment();
 while( !(r.right == cur || r.left == cur) ) {
 if(r.right.symbol != ' '){
 if(r.right.number > cur.number && cur.count > r.right.count){ alg1=true;
...
content.put(r.left.symbol,r.left);
 ...
 } public void Increment(){ Node r = NYT;
 while ( !(r == root ) ) {
 ...
 for (char i : dic.keySet()) {if (dic.get(i).matches(shortcode)) return i;} return ' '; }
